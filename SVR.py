...
# 6. SVR Model
svr_rbf = SVR(kernel='rbf', C=10, gamma=0.0001, epsilon=0)
svr_rbf.fit(stand_X, Y)
pred_Y = svr_rbf.predict(stand_X)
pred_Y[pred_Y<0] = 0
print("RMSLE score = ", np.sqrt(mean_squared_log_error(Y, pred_Y)))

# 7. Submit
prediction = svr_rbf.predict(stand_test_X)
prediction[prediction<0] = 0
prediction = pd.DataFrame(prediction, columns=['Votes'])
prediction.insert(0, 'Id', test_set_review['review_id'])
prediction.to_csv('submit/SVR.csv', index=False)
