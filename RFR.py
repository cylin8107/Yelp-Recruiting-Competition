...
# -*- coding: utf-8 -*-
"""
Data Mining final project: Random Forest Regressor
...
# 6. GradientBoostingRegressor Model
RFR = RandomForestRegressor(n_estimators=5, max_depth=5, min_samples_split=3, random_state=0).fit(stand_X, Y)
pred_Y = RFR.predict(stand_X)
pred_Y[pred_Y<0] = 0
print("RMSLE score = ", np.sqrt(mean_squared_log_error(Y, pred_Y)))

# 7. Submit
prediction = RFR.predict(stand_test_X)
prediction[prediction<0] = 0
prediction = pd.DataFrame(prediction, columns=['Votes'])
prediction.insert(0, 'Id', test_set_review['review_id'])
prediction.to_csv('submit/RFR.csv', index=False)